...
from aiowialon import flags, WialonCallRespValidator
...
async def direct_upload_driver_image():
    ...
    with open(file_path, "rb") as f:
        form_data.add_field(
            "n", f.read(), filename="image.jpg", content_type="image/jpeg"
        )
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, data=form_data) as response:
            ...
            await WialonCallRespValidator.validate_result(
                "drivers_dlg_props_upload_image", res
            )
            print(res)
