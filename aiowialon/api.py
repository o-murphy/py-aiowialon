...
class Wialon:
    ...
    def __init__(
        ...
        self.__base_url = (
            f"{scheme}://{host}:{port if port else 443 if scheme == 'https' else 80}"
        )
    ...
    def on_session_open(
        ...
        if callback and not callable(callback):
            raise TypeError(
                f"'on_session_open' callback must be a type of {ClientLoginCallback}"
            )
    ...
    def on_session_close(
        ...
        if callback and not callable(callback):
            raise TypeError(
                f"'on_session_close' callback must be a type of {ClientLogoutCallback}"
            )
    ...
    async def start_polling(
        ...
        **params: Unpack[ClientLoginParams],
        ...
        if timeout < 1:
            raise ValueError(
                "Poling timeout have to be >= 1 second. "
    ...
    async def login(
        self, **params: Unpack[ClientLoginParams]
    ) -> token_params.TokenLoginResponse:
        ...
        if token and auth_hash:
            raise ValueError(
                "You can't use both token and auth_hash at the same time on login"
            )
    ...
    async def avl_evts(self) -> other.AvlEventResponse:
        ...
        if self.__polling_task:
            warnings.warn(
                "Polling running, don't recommended to call 'avl_evts' manually",
                WialonWarning,
            )
    ...
    async def batch(
        self,
        *calls: Coroutine[Any, Any, Any],
        flags_: flags.BatchFlag = flags.BatchFlag.EXECUTE_ALL,
    ) -> core.CoreBatchResponse:
        ...
        for coroutine in calls:
            ...
            actions.append(
                {
                    "svc": convention.prepare_action_name(
                        coroutine_locals["action_name"]
                    ),
    ...
    async def multipart(
        self, call: Coroutine[Any, Any, Any], *fields: MultipartField
    ) -> Any:
    ...
    def help(service_name: str, action_name: str) -> None:
        ...
        try:
            ...
            webbrowser.open(
                url.format(service_name=service_name, action_name=action_name)
            )
        except ImportError:
