...
class WLP:
    ...
    async def export_item(client: Wialon, item_id: int) -> bytes:
        ...
        response = await client.core_search_item(
            id=item_id, flags=flags.UnitsDataFlag.ALL
        )
        item = response["item"]
        item_type = item["cls"]
        ...
        return await client.exchange_export_json(
            fileName=f"{item['id']}", json=wlp_data
        )
    ...
    async def _fetch_unit_wlp(client: Wialon, item: dict[str, Any]) -> dict[str, Any]:
        ...
        if len(hw_types) <= 0:
            raise WialonInvalidInput("Hardware type not found", "wlp.export_unit", "")
        hw_type = hw_types[0]
        commands_ids = [str(i) for i in range(1, 15)]

        response = await client.batch(
            client.unit_update_hw_params(
                itemId=unit_id, hwId=hw_type["id"], fullData=1, action="get"
            ),
        ...
        wlp_data["reportProps"]["fuelConsRates"] = item["rfc"][
            "fuelConsRates"
        ]  # ? changed
    ...
    async def _fetch_resource_wlp(
        ...
        response = await client.batch(
            client.resource_get_zone_data(
                itemId=resource_id, col=zones_ids, flags=0x1C
            ),
